function KF_in = model_KF2(gout,x)
...
H(1,[y yL z])   = [100 -100 100];
H(2,[p     ])   = 100; % SW's data set quarterly inflation (originally annualized in HS)
H(3,[R     ])   = 100; % SW's data set quarterly FFR

H = H*gout.z;

C = [gammQ; pA; (pA+rA+4*gammQ)];
...
shatinit = zeros(4+1,1);
    shatinit(4+1) = 1; % constant term
